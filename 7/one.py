...
class Register:
    ...
    def input_instruction(self):
        ...
        assert self._index < len(self.input)
...
def save(ip, program, modes):
    p = program.copy()
    p[p[ip + 1]] = master_register.input_instruction()
    return (ip + 2, p)
...
def thruster_signal(program, seq):
    ...
    for i in range(5):
        input = [seq[i], master_register.output]
        execute(program, input)
    return master_register.output

assert 43210 == thruster_signal([3,15,3,16,1002,16,10,16,1,16,15,15,4,15,99,0,0], [4,3,2,1,0])
assert 54321 == thruster_signal([3, 23, 3, 24, 1002, 24, 10, 24, 1002, 23, -1, 23, 101, 5, 23, 23, 1, 24, 23, 23, 4, 23, 99, 0, 0], [0,1,2,3,4])
assert 65210 == thruster_signal([3, 31, 3, 32, 1002, 32, 10, 32, 1001, 31, -2, 31, 1007, 31, 0, 33, 1002, 33, 7, 33, 1, 33, 31, 31, 1, 32, 31, 31, 4, 31, 99, 0, 0, 0], [1,0,4,3,2])
